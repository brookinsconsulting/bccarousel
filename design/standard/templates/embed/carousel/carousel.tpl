{**
 * File containing the feature_scroller template
 *
 * @package stdl
 * @version //autogentag//
...
<div class="feature_scroller_wrapper">
...
{/literal}
    ...
    thisCarousel[carousel_unique_id].init('carouselContainer_' + carousel_unique_id, 'carouselInner_' + carousel_unique_id, 'prevBtn_' + carousel_unique_id, 'nextBtn_' + carousel_unique_id, {$childrenCount}, 3, {if $object.data_map.autoscroll.value|eq( '1' )}true{else}false{/if}, {if $object.data_map.autoscroll_hold_time.has_content|eq( 'true' )}{$object.data_map.autoscroll_hold_time.value}{else}3000{/if}, 20, 50, thisCarousel[carousel_unique_id], carousel_unique_id);
{literal}
});*/
{/literal *}
{* </script> *}
...
<ul id="carousel3wide{$object.main_node_id}" class="jcarousel-skin-3wide">
...
{* </div> *}
{* <!-- New Structure
 <ul id="mycarousel" class="jcarousel-skin-3wide">
...
--> *}
<li>{if $object.data_map.show_titles.value|eq('1')}<h3>{$child_item_link_open}{$child_item_name}{$child_item_link_close}</h3>{/if}{$child_item_link_open}{if ne($child_image_url, '')}<img alt="{$child.name}" src={$child_image_url|ezurl()}{if and(is_set($child.data_map.picheight), gt(min($child.data_map.picheight.content, $maximum_height), 0))} height="{min($child.data_map.picheight.content, $maximum_height)}"{/if}{if and(is_set($child.data_map.picwidth), gt($child.data_map.picwidth.content, 0), gt($maximum_height, $child.data_map.picheight.content))} width="{$child.data_map.picwidth.content}"{/if} />{$child_item_link_close}{/if}</li>{* <div class="bc_carousel">{$child_item_link_open}{if ne($child_image_url, '')}<img alt="{$child.name}" src={$child_image_url|ezurl()}{if and(is_set($child.data_map.picheight), gt(min($child.data_map.picheight.content, $maximum_height), 0))} height="{min($child.data_map.picheight.content, $maximum_height)}"{/if}{if and(is_set($child.data_map.picwidth), gt($child.data_map.picwidth.content, 0), gt($maximum_height, $child.data_map.picheight.content))} width="{$child.data_map.picwidth.content}"{/if} />{$child_item_link_close}{/if}{if $object.data_map.show_titles.value|eq('1')}<div class="carousel_title">{$child_item_link_open}{$child_item_name}{$child_item_link_close}</div>{/if}</div> *}
{/for}
</ul>

<script language="Javascript" type="text/javascript">
jQuery(document).ready(function() {literal}{{/literal}
jQuery("#carousel3wide{$object.main_node_id}").jcarousel();
{literal}}{/literal});
    /*carousel_unique_namespace('carouselContainer{$object.main_node_id}', 'carouselInner', 'prevBtn', 'nextBtn');
